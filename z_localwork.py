...
async def save(ctx):
    ...
    try:
        ...
        print("config saved!")
        print(server_dict[ctx.message.channel.server])
    except Exception as err:
        print(_("Error occured while trying to save!"))
        print(err)
...
async def _exraid(message):
    ...
    raidmsg = _("""Clembot! {pokemon} EX raid reported by {member}! Details: {location_details}. Coordinate here!
...
async def _eggtoraid(entered_raid, raid_channel):
    ...
    raidmsg = _("""Clembot! The egg hatched into a {pokemon} raid! Details: {location_details}. Coordinate here!
...
async def interested(ctx, *, count: str = None):
    ...
    if count:
        if count.isdigit():
            ...
            await Meowth.send_message(ctx.message.channel, _("Clembot! I can't understand how many are in your group. Just say `!interested` if you're by yourself, or `!interested 5` for example if there's 5 in your group."))
            return
    else:
...
async def coming(ctx, *, count: str = None):
    ...
    if count:
        if count.isdigit():
            ...
            await Meowth.send_message(ctx.message.channel, _("Clembot! I can't understand how many are in your group. Just say `!coming` if you're by yourself, or `!coming 5` for example if there's 5 in your group."))
            return
    else:
        if ctx.message.author.mention in trainer_dict:
            count = trainer_dict[ctx.message.author.mention]['count']
...
async def here(ctx, *, count: str = None):
    ...
    if count:
        if count.isdigit():
            ...
            await Meowth.send_message(ctx.message.channel, _("Clembot! I can't understand how many are in your group. Just say `!here` if you're by yourself, or `!coming 5` for example if there's 5 in your group."))
            return
    else:
        if ctx.message.author.mention in trainer_dict:
            count = trainer_dict[ctx.message.author.mention]['count']
...
@checks.activeraidchannel()
async def omw(ctx):
    await Meowth.send_message(ctx.message.channel, content = _("Clembot! Hey {member}, I don't know if you meant **!coming** to say that you are coming or **!otw** to see the other trainers on their way").format(member=ctx.message.author.mention))
