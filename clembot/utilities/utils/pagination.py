...
class Pagination:
    ...
    def __init__(self, ctx, entries, *, per_page=12, show_entry_count=True,
                 title='Help', msg_type='help', plain_text=False):
        self.bot = ctx.bot
        ...
        self.embed = Embeds.make_embed(title=title, msg_type=msg_type)
    ...
    async def show_page(self, page, *, first=False):
        ...
        #     else:
        #         self.embed.set_footer(text=('Use {}help <command> for more details.').format(self.prefix))
        # else:
        #     self.embed.set_footer(text=('Use {}help <command> for more details.').format(self.prefix))
        ...
        if entries:
            ...
            for entry in entries:
                cmd_msg += (
                    "{}\n"
                ).format(signature(entry) if not self.plain_text else entry)
            self.embed.add_field(
    ...
    @staticmethod
    async def from_list(cls, ctx, list, title):
        self = cls(ctx, list)
        self.title = f'{title}'
        self.description = None
...
class TextPagination:
    ...
    """
    def __init__(self, ctx, entries, *, per_page=12, show_entry_count=True,
                 title='Help', msg_type='help', plain_text=False, embed_header=None):
        self.bot = ctx.bot
        self.entries = entries
        self.message = ctx.message
        self.channel = ctx.channel
        ...
        self.description=None
        self.plain_text=plain_text
        self.embed_header=embed_header or title
        self.embed = Embeds.make_embed(title=title, msg_type=msg_type)
        self.default_icon = self.embed.author.icon_url
        self.paginating = len(entries) > per_page
        self.show_entry_count = show_entry_count
    ...
    async def show_page(self, page, *, first=False):
        ...
        self.embed.title = self.title
        self.embed.description = self.description
        self.embed.set_footer(text=f"Page {page} of {self.maximum_pages}")
        ...
        if entries:
            ...
            for entry in entries:
                cmd_msg += f"{entry}\n"
            self.embed.add_field(
                name=f"{self.embed_header}", value=f"{cmd_msg}", inline=False)
...
class CommandHelpPagination(TextPagination):
