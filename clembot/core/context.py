...
class Context(commands.Context):
    ...
    async def refresh_embed(self, message_or_reference_id, new_embed_data):
        ...
        plain_msg =  new_embed_data.get('plainText', None)
        msg_type = "warning"
        # title_url = new_embed_data.get('color', None)
        colour=new_embed_data.get('color', None)
        icon=new_embed_data.get('icon', None)
        # thumbnail=new_embed_data.get('thumbnail', None)
        image=new_embed_data.get('image', {}).get('url', None)
        footer=new_embed_data.get('footer', {}).get('text', None)

        new_embed = await self.embed(title=title, description=description, plain_msg=plain_msg, colour=colour, icon=icon, image=image, footer=footer, send=False)
