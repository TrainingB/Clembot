...
class BingoCog(commands.Cog):
    ...
    async def cmd_bingo_card_original(self, ctx):
        return await Embeds.error(ctx.channel, f"This command has been migrated to `!bingo card`.",
    ...
    async def cmd_bingo_card(self, ctx):
        ...
        try:
            ...
            if event_pokemon is None:
                return await Embeds.error(message.channel,
                                          f"The bingo-event is not set yet. Please contact an admin.**",
                                          user=message.author)
