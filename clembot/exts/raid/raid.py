...
class RSVPEnabled:
    ...
    @property
    def safe_trainer_dict(self):
        self.trainer_dict = self.trainer_dict or {}
        return self.trainer_dict

    # @property
    # def trainer_dict(self):
    #     return self._trainer_dict or {}

    # @trainer_dict.setter
    # def trainer_dict(self, trainer_dict):
    #     self._trainer_dict = trainer_dict or {}
    ...
    async def add_rsvp(self, member_id: str, status, count=None):
        ...
        self.trainer_dict[member_id] = { 'status': status, 'count' :count }
    ...
    async def send_remote_rsvp_embed(self, message, description=None, options=['timer', 'rsvp']):

        embed_message = await message.channel.send(embed=await self.remote_rsvp_embed(message, options=options,
                                                                           description=description))
        await embed_message.add_reaction('🗑️')

    async def remote_rsvp_embed(self, message, options=None, description=None):
        ...
        for option in options:
            ...
            if option == 'rsvp':
                int_status = ""
                trainer_names = await self.trainers_details_by_status(message, 'ir')
                if trainer_names:
                    int_status += f"Interested in remote invite {MyEmojis.REMOTE}\n"

                    additional_fields[int_status] = trainer_names

        return Embeds.make_embed(header="Remote Raid Members", msg_color=discord.Color.gold(), fields=additional_fields, footer=footer, content=description)
    ...
    async def trainers_details_by_status(self, message, status, mentions=False, delimiter='\n'):
        Logger.info("trainers_details_by_status")
        name_list = []
        for trainer in self.safe_trainer_dict.keys():
            if self.trainer_dict[trainer]['status'] == status:
                user = message.channel.guild.get_member(int(trainer))

                user_profile = await UserProfile.find(self.bot, user.id)
                ign = '/'.join(_.get(user_profile,'ign'))
                trainer_code = '/'.join(_.get(user_profile,'trainer_code'))
                
                display_name = f"{user.mention} - {ign} ({trainer_code})"
                
                name_list.append(f"**{display_name}**")
...
class Raid (RSVPEnabled):
    ...
    def __init__(self, raid_id=None, bot=None, guild_id=None, channel_id=None, author_id=None,
        ...
        self.actions = [MyEmojis.POKE_BATTLER, MyEmojis.REMOTE]
        self.snowflake = Snowflake()
...
class RemoteRaidEmbed:
    ...
    def from_raid(cls, raid: Raid):
        ...
        remote_raid_info = f"Tap {MyEmojis.REMOTE} to join."
        ...
        fields = {
            ...
            "**Join Remote Raid**": remote_raid_info,
            # "**Pokebattler says**": [False, pb_raid_party_stats],
            # "**Suggested Start**" : start,
        }

        embed = Embeds.make_embed(header="Remote Raid", header_icon=Icons.raid_report, thumbnail=img_url, fields=fields,
...
class RemoteEggEmbed:
    ...
    def from_raid(cls, raid: Raid):
        ...
        embed = Embeds.make_embed(header="Remote Raid Report", header_icon=Icons.raid_report, thumbnail=img_url, fields=fields,
                                  footer=footer, footer_icon=footer_icon, msg_color=color)
...
class RemoteRaid(object):
    pass


def create_overwrite_from(channel, user):
    allow = [(permission[0], True) for permission in channel.permissions_for(user) if permission[1]]
    deny = [(permission[0], True) for permission in channel.permissions_for(user) if not permission[1]]
...
class DiscordOperations:
    ...
    async def create_private_channel(self, ctx, remote_raid: RemoteRaid):
        try:
            channel, message = await ChannelMessage.from_text(self.bot, remote_raid.report_message)

            guild = ctx.guild
            member = ctx.author
            
            author = await ctx.guild.fetch_member(remote_raid.author_id)
            
            overwrites = {
                guild.default_role: discord.PermissionOverwrite(read_messages=False),
                guild.me: create_overwrite_from(ctx.channel, guild.me),
                author: create_overwrite_from(ctx.channel, author),
            }
            raid_channel = await guild.create_text_channel(remote_raid.channel_name, overwrites=overwrites, category=ctx.guild.get_channel(channel.category_id))
...
class RemoteRaid (RSVPEnabled):
    ...
    REMOTE_LOCATION = POILocation(location="", city="", gym=None, url=None )
    

    def __init__(self, raid_id=None, bot=None, guild_id=None, channel_id=None, author_id=None,
                 report_message: str = None, raid_type=None, level=None,
                 pkmn: Pokemon = None, timer=None, reported_time=None, hatch_time=None, expiry_time=None,
                 start_time=None, response_message: str = None, channel_message: str = None, timezone=None,
                 poke_battler_id=None, pb_raid_info=None, trainer_dict=None):
        """
        ...
        self.level = level
        self.pkmn = pkmn
        self.raid_location = RemoteRaid.REMOTE_LOCATION
        # TODO: Connect guild timezone
        self.timezone = timezone
    ...
    def channel_name(self):
        ...
        status = self.status
        if status in ["hatched", "expired"]:
            channel_name = f"{self.status}-{raid_boss}{raid_id}"
        else:
            channel_name = f"{raid_boss}{raid_id}"
    ...
    async def egg_embed(self):
        return (RemoteEggEmbed.from_raid(self)).embed

    async def raid_embed(self):
        return (RemoteRaidEmbed.from_raid(self)).embed


    async def expired_embed(self):
        author = await self.guild.fetch_member(self.author_id)

        raid_embed = Embeds.make_embed(content=f"This remote raid has expired!", msg_color=color())
        raid_embed.set_footer(text=f"Reported by {author.display_name} | Ended at {self.expires_at}")
