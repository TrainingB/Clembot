...
def custom_error_handling(bot,logger):
    ...
    async def on_command_error(error, ctx):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            msg = "Beep Beep! Please use **!{cmd_name}** in one of the following region channels:".format(cmd_name=ctx.command.name)
            ...
            server = ctx.message.server
            msg = "Beep Beep! Please use **!{cmd_name}** in a Raid channel. Use **!list** in any of the following region channels to see active raids:".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server]['city_channels']
            for c in city_channels:
                channel = discord.utils.get(server.channels,name=c)
                msg += "\n" + channel.mention
            ...
            server = ctx.message.server
            msg = "Beep Beep! Please use **!{cmd_name}** in an Active Raid channel. Use **!list** in any of the following region channels to see active raids:".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server]['city_channels']
            for c in city_channels:
                channel = discord.utils.get(server.channels,name=c)
                msg += "\n" + channel.mention
            ...
            server = ctx.message.server
            msg = "Beep Beep! Please use **!{cmd_name}** in either a Raid channel or one of the following region channels:".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server]['city_channels']
            for c in city_channels:
                channel = discord.utils.get(server.channels,name=c)
                msg += "\n" + channel.mention
            ...
            server = ctx.message.server
            msg = "Beep Beep! Please use **!{cmd_name}** in either a Raid Egg channel or one of the following region channels:".format(cmd_name=ctx.command.name)
            city_channels = bot.server_dict[server]['city_channels']
            for c in city_channels:
                channel = discord.utils.get(server.channels,name=c)
                msg += "\n" + channel.mention
