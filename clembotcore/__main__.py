...
from clembotcore.exts.argparser import ArgParser
from clembotcore.exts.bingo import card_data_generator
from clembotcore.exts.bingo.bingo_cog import BingoCardGenerator
from clembotcore.exts.pkmn.pkmn_cog import Pokemon
...
def load_config():
    ...
    # language = gettext.translation('clembotcore', localedir='locale', languages=[config_template.bot_language])
    # language.install()
...
default_exts = ['exts.silph','exts.propertieshandler', 'exts.utilities', 'exts.trademanager',
                ...
                'exts.badgemanager','exts.gymmanager']
...
async def get_city_list(message):
    city_list = []
    try:
        ...
        city_channel = await MyGymManager.get_city_for_channel(message.guild.id, message.channel.id)

        if city_channel:
            city_list.append(city_channel)
        else:
            guild_channel = city_channel = await MyGymManager.get_city_for_channel(message.guild.id, None)
            if guild_channel:
...
def clean_quick_move(pokemon_type:str):
    return pokemon_type.replace('_FAST','').replace('_',' ').title()

def clean_cinematic_move(pokemon_type:str):
    return pokemon_type.replace('_',' ').title()



@Clembot.command(pass_context=True, hidden=True, aliases=["info"])
async def _info(ctx, *pokemon_list):

    mon_list = []
    for pokemon in pokemon_list:
        mon_info = {}
        for mon in pokemon_master['pokemon']:
            try:
                if mon['pokedex']['pokemonId'] == pokemon.upper():

                    mon_info['pokemon'] = mon['pokedex']['pokemonId'].title()


                    mon_info.setdefault('type',[]).append(clean_type(mon['type']))
                    if mon.get('type2',False):
                        mon_info.setdefault('type', []).append(clean_type(mon['type2']))

                    mon_info['cinematicMoves'] = [clean_cinematic_move(move) for move in mon['cinematicMoves']]
                    mon_info['quickMoves']=[clean_quick_move(move) for move in mon['quickMoves']]

                    mon_list.append(mon_info)
            except Exception as error:
                await _send_message(ctx.channel, error)

    await _send_message(ctx.channel, json.dumps(mon_list))
...
async def mysetup(ctx):
    ...
    raid_embed.add_field(name="**Username:**", value=("{option}").format(option=user.name))
    raid_embed.add_field(name="**Roles:**", value=("{roles}").format(roles=" \ ".join(text)))
    raid_embed.set_thumbnail(url=("https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.{format}".format(user=user, format="jpg")))
...
async def on_ready():
    ...
    await _print(Clembot.owner, ("Starting up..."))  # prints to the terminal or cmd prompt window upon successful connection to Discord
    ...
    embed = discord.Embed(colour=discord.Colour.green(), description="Beep Beep! That's right!").set_author(name=("Clembot Startup Notification"), icon_url=Clembot.user.avatar_url)
    embed.add_field(name="**Servers Connected**", value=(" {guilds}").format(guilds=guilds), inline=True)
    embed.add_field(name="**Members Found**", value=(" {members}").format(members=users), inline=True)
...
async def outputlog(ctx):
    ...
    with open(os.path.join('logs', 'clembotcore.log'), 'r') as logfile:
...
async def announce(ctx, *, announce=None):
    """Repeats your message in an embed from clembotcore.
...
async def configure(ctx):
        ...
        while True:
            ...
            if citychannels.content.lower() == 'n':
                ...
                if (not diff):
                    ...
                    await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=("The channel list you provided doesn't match with your servers channels.\n\nThe following aren't in your server: {invalid_channels}\n\nPlease double check your channel list and resend your reponse.").format(invalid_channels=', '.join(diff))))
                    continue
    if (configcancel == False) and (guild_dict_temp['other'] == True) and ((guild_dict_temp['wildset'] == True) or (guild_dict_temp['raidset'] == True)) and ((firstconfig == True) or (configgoto == 'all') or (configgoto == 'regions') or (configgoto == 'allmain')):
            ...
            if categories.content.lower() == 'cancel':
                ...
                while True:
                    ...
                    if len(levelcat_list) == 6:
                        ...
                        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=("The number of categories doesn't match the number of raid levels! Make sure you give me exactly six categories, one for each level of raid. You can use the same category for multiple levels if you want, but I need to see six category names.")))
                        continue
            else:
                    ...
                    if (not diff):
                        ...
                        while True:
                            try:
                                ...
                                logger.info(error)
                                await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=("You didn't give me enough permissions to create channels! Please check my permissions and that my role is above general roles. Let me know if you'd like me to check again.\n\nRespond with: **Y** to try again, or **N** to skip and create the missing channels yourself.")))
                                while True:
    ...
    if (configcancel == False) and (guild_dict_temp['other'] == True) and (guild_dict_temp['raidset'] == True) and ((firstconfig == True) or (configgoto == 'all') or (configgoto == 'timezone') or (configgoto == 'allmain')):
        await owner.send(embed=discord.Embed(colour=discord.Colour.lighter_grey(), description=("To help coordinate raids reports for you, I need to know what timezone you're in! The current 24-hr time UTC is {utctime}. How many hours off from that are you?\n\nRespond with: A number from **-12** to **12**:").format(utctime=strftime('%H:%M', time.gmtime()))).set_author(name='Timezone Configuration', icon_url=Clembot.user.avatar_url))
        while True:
...
async def all(ctx):
    ...
    if count == 0:
        await channel.send( content=("{0}, you have no pokemon in your want list.").format(author.mention, count))
        return
    await channel.send( content=("{0}, I've removed {1} pokemon from your want list.").format(author.mention, count))
    return
...
async def _wild(message):
    try:
        ...
        if len(wild_split) <= 1:
            ...
            wild_embed = discord.Embed(title=("Beep Beep! Click here for my directions to the wild {pokemon}!").format(pokemon=entered_wild.capitalize()), description=("Ask {author} if my directions aren't perfect!").format(author=message.author.display_name), url=wild_gmaps_link, colour=message.guild.me.colour)
            ...
            wildreportmsg = await message.channel.send(content=("Beep Beep! Wild {pokemon} reported by {member}! Details: {location_details}").format(pokemon=title_or_mention, member=message.author.mention, location_details=wild_details) , embed=wild_embed)
...
async def _find_gym(message):
    ...
    try :
        ...
        if 0 < len(args_split) < 3 :
            ...
            return await message.channel.send(content=("Beep Beep! {member} {message}".format(member=message.author.mention, message=roster_message)), embed=raid_embed)
        else:
...
registers a role and a gym
    ...
    if len(args) == 0:
        await channel.send( content=("Beep Beep! No role-name provided. Please use `!register-role role-name` to create/register the role!"))
    ...
    await channel.send( content=("Beep Beep! {role_name} has been registered for notifications. Please use `!register-gym role-name gym-code` to register the gym under a role!").format(role_name=role.mention))
...
registers a role and a gym
    ...
    if len(args) == 0:
        await channel.send( content=("Beep Beep! No role-name provided. Please use `!deregister-role role-name` to create/register the role!"))
    ...
    await channel.send( content=("Beep Beep! {role_name} has been de-registered for notifications.").format(role_name=role.mention))
...
registers a role and a gym
    ...
    # Create role if it doesn't exist yet
    if role is None:
        await channel.send( content=("Beep Beep! I couldn't find the role **{role_name}**. Please use `!register-role role-name` to create/register the role!".format(role_name=role_name)))
        return

    if role.id not in guild_dict[guild.id]['notifications']['roles']:
        await channel.send( content=("Beep Beep! The role {role_name} is not registered for notifications. Please use `!register-role role-name` to create/register the role!"))
        return

    if len(args) == 0 or len(args) > 1:
        await channel.send( content=("Beep Beep! Please provide a gym-code to register. `!register role-name gym-code`"))
        return
    ...
    if gym_info == None:
        await channel.send( content=("Beep Beep! Hmmm... I could not find this gym code!"))
        return
    ...
    await channel.send( content=("Beep Beep! {gym_code} will not send out a notification for {role}!").format(gym_code=gym_code.upper(), role=role.mention))
...
async def _contest(message):
    try:
        ...
        await message.channel.send( content=("Beep Beep! A contest is about to take place in {channel}!".format(channel=contest_channel.mention)))

        raid_embed = discord.Embed(title=("Beep Beep! A contest is about to take place in this channel!"), colour=discord.Colour.gold(), description="The first member to correctly guess (and spell) the randomly selected pokemon name will win!")
        ...
        raid_embed.set_footer(text=("Reported by @{author}").format(author=message.author.display_name), icon_url=message.author.avatar_url)
        ...
        embed = discord.Embed(colour=discord.Colour.gold(), description="Beep Beep! A contest channel has been created!").set_author(name=("Clembot Contest Notification - {0}").format(message.guild), icon_url=Clembot.user.avatar_url)
        ...
        embed.add_field(name="**Server:**", value=("{member}").format(member=message.guild.name), inline=True)
        embed.add_field(name="**Reported By:**", value=("{member}").format(member=message.author.display_name), inline=True)
        await Clembot.owner.send(embed=embed)
    ...
    if 'contest_channel' in guild_dict[message.guild.id]:
        if guild_dict[message.guild.id]['contest_channel'][message.channel.id].get('started', True) == False:
            if ctx.message.author.id == guild_dict[message.guild.id]['contest_channel'][message.channel.id].get('reported_by', 0):
                ...
                raid_embed = discord.Embed(title=("Beep Beep! The channel is open for submissions now!"), colour=discord.Colour.gold())
...
async def contestEntry(message, pokemon=None):
    ...
    if pokemon.lower() == message.content.lower():
        ...
        await message.channel.send( content=("Beep Beep! Congratulations {winner}!").format(winner=message.author.mention))
...
async def _newraid(message):
    ...
    raid_embed = discord.Embed(colour=message.guild.me.colour) # title=("Beep Beep! Click here for directions to the raid!"), url=raid_gmaps_link,
    raid_embed.add_field(name="**Where:**", value=direction_description, inline=False)
    raid_embed.add_field(name="**Details:**", value=("{pokemon} ({pokemonnumber}) {type}").format(pokemon=entered_raid.capitalize(), pokemonnumber=str(raid_number), type="".join(get_type(message.guild, raid_number)), inline=True))
    raid_embed.add_field(name="**Weaknesses:**", value=("{weakness_list}").format(weakness_list=weakness_to_str(message.guild, get_weaknesses(entered_raid))), inline=True)
    raid_embed.set_footer(text=("Reported by @{author}").format(author=message.author.display_name), icon_url=("https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.{format}?size={size}".format(user=message.author, format="jpg", size=32)))
...
""").format(pokemon=raid_role, member=message.author.mention, citychannel=message.channel.mention, location_details=raid_details)
    ...
    if channel_role_list:
        await raid_channel.send( content=("Beep Beep! A raid has been reported for {channel_role}.").format(channel_role=channel_role.mention))

    if raidexp is not False:
        await _timerset(raid_channel, raidexp)
    else:
        await raid_channel.send( content=("Beep Beep! Hey {member}, if you can, set the time left on the raid using **!timerset <minutes>** so others can check it with **!timer**.").format(member=message.author.mention))
...
async def _raid(message):
    ...
    raid_img_url = "https://raw.githubusercontent.com/FoglyOgly/Clembot/master/images/pkmn/{0}_.png".format(str(raid_number).zfill(3))
    raid_img_url = get_pokemon_image_url(raid_number)  # This part embeds the sprite
    raid_embed = discord.Embed(title=("Beep Beep! Click here for directions to the raid!"), url=raid_gmaps_link, colour=message.guild.me.colour)
    raid_embed.add_field(name="**Details:**", value=("{pokemon} ({pokemonnumber}) {type}").format(pokemon=entered_raid.capitalize(), pokemonnumber=str(raid_number), type="".join(get_type(message.guild, raid_number)), inline=True))
    raid_embed.add_field(name="**Weaknesses:**", value=("{weakness_list}").format(weakness_list=weakness_to_str(message.guild, get_weaknesses(entered_raid))), inline=True)
    raid_embed.set_footer(text=("Reported by @{author}").format(author=message.author.display_name), icon_url=("https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.{format}?size={size}".format(user=message.author, format="jpg", size=32)))
    raid_embed.set_thumbnail(url=raid_img_url)
...
""").format(pokemon=raid_role, member=message.author.mention, citychannel=message.channel.mention, location_details=raid_details)
    ...
    if channel_role_list:
        ...
        # if channel_mentions:
        #     await raid_channel.send(content=("Beep Beep! A raid has been reported for {channel_role}.").format(channel_role=channel_mentions))
...
async def old_fetch_counters_dict(pkmn , weather:None):
    ...
    try:
        ...
        # for moveset in ctrs_dict:
            ...
            # ctrs_dict[moveset]['embed'].add_field(name=("Results with Level 30 attackers"), value=("[See your personalized results!](https://www.pokebattler.com/raids/{pkmn})").format(pkmn=pkmn.replace('-','_').upper()),inline=False)
    ...
    try:
        ...
        while True:
            if args:
                ...
                research_embed.add_field(name=("**Quest:**"),value='\n'.join(textwrap.wrap(quest.title(), width=30)),inline=True)
                research_embed.set_field_at(0, name=research_embed.fields[0].name, value=("Fantastic! Now, reply with the **reward** for the **{quest}** quest that you received from **{location}**. You can reply with **cancel** to stop anytime.\n\nHere's what I have so far:").format(quest=quest, location=location), inline=False)
        ...
        if not error:
            ...
            research_embed.add_field(name='**Research Report Cancelled**', value=("Beep Beep! Your report has been cancelled because you {error}! Retry when you're ready.").format(error=error), inline=False)
            confirmation = await channel.send(embed=research_embed)
            await asyncio.sleep(10)
            await confirmation.delete()
            await message.delete()
...
async def process_map_link(message, newloc=None):
    ...
    await message.channel.send( content=("Beep Beep! Someone has suggested a different location for the raid! Trainers {trainer_list}: make sure you are headed to the right place!").format(trainer_list=", ".join(otw_list)), embed=newembed)
...
async def __exraid(ctx):
    ...
    try:
        ...
        logger.info(error)
        await message.channel.send(content=("Beep Beep! An error occurred while creating the channel. {error}").format(error=error))
        return

    raid_img_url = get_egg_image_url(egg_level)
    raid_embed = discord.Embed(title=("Beep Beep! Click here for directions to the coming raid!"), url=raid_gmaps_link, colour=message.guild.me.colour)
...
async def _exraid(ctx):
    ...
    raid_embed.set_footer(text=("Reported by @{author}").format(author=message.author.display_name), icon_url=("https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.{format}?size={size}".format(user=message.author, format="jpg", size=32)))
    raid_embed.set_thumbnail(url=raid_img_url)
    raidreport = await channel.send( content=("Beep Beep! EX raid egg reported by {member}! Details: {location_details}. Use the **!invite** command to gain access and coordinate in {raid_channel}").format(member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention), embed=raid_embed)
    await asyncio.sleep(1)  # Wait for the channel to be created.
...
Message **!starting** when the raid is beginning to clear the raid's 'here' list.""").format(member=message.author.mention, citychannel=channel.mention, location_details=raid_details)
    ...
    await raid_channel.send( content=("Beep Beep! Hey {member}, if you can, set the time the EX Raid begins using **!timerset <date and time>** so others can check it with **!timer**. **<date and time>** should look exactly as it appears on your invitation.").format(member=message.author.mention))
...
async def _raidegg(message):
    try:
        ...
        if egg_level > 5 or egg_level == 0:
            ...
            raid_embed = discord.Embed(colour=message.guild.me.colour)
            raid_embed.add_field(name="**Where:**", value=direction_description, inline=False)
            if len(egg_info['pokemon']) > 1:
                raid_embed.add_field(name="**Possible Bosses:**", value=("{bosslist1}").format(bosslist1="\n".join(boss_list[::2])), inline=True)
                raid_embed.add_field(name="\u200b", value=("{bosslist2}").format(bosslist2="\n".join(boss_list[1::2])), inline=True)
            ...
            raid_embed.set_thumbnail(url=raid_img_url)
            try:
                raidreport = await message.channel.send(content=("Beep Beep! Level {level} raid egg reported by {member}! Details: {location_details}. Coordinate in {raid_channel}").format(level=egg_level, member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention), embed=raid_embed)
            except Exception as error:
                logger.info(error)
            await asyncio.sleep(1)  # Wait for the channel to be created.
    ...
    """).format(level=egg_level, member=message.author.mention, citychannel=message.channel.mention, location_details=raid_details)
            ...
            if raidexp is not False:
                ...
                await raid_channel.send(content=("Beep Beep! Hey {member}, if you can, set the time left until the egg hatches using **!timerset <minutes>** so others can check it with **!timer**.").format(member=message.author.mention))
            ...
            if len(raid_info['raid_eggs'][egg_level]['pokemon']) == 1:
                await _eggassume("assume " + get_name(raid_info['raid_eggs'][egg_level]['pokemon'][0]), raid_channel)
            elif egg_level == "5" and get_number(guild_dict[raid_channel.guild.id]['configuration'].get('settings',{}).get('regional', None)) in raid_info['raid_eggs']["5"]['pokemon']:
...
async def _eggtoraid(entered_raid, channel):
    try:
        ...
        raid_embed = discord.Embed(colour=channel.guild.me.colour)  # title=("Beep Beep! Click here for directions to the raid!"), url=raid_gmaps_link,
        raid_embed.add_field(name="**Where:**", value=where.value, inline=False)
        # raid_embed = discord.Embed(title=("Beep Beep! Click here for directions to the raid!"), url=raid_gmaps_link, colour=channel.guild.me.colour)
        raid_embed.add_field(name="**Details:**", value=("{pokemon} ({pokemonnumber}) {type}").format(pokemon=entered_raid.capitalize(), pokemonnumber=str(raid_number), type="".join(get_type(channel.guild, raid_number)), inline=True))
        raid_embed.add_field(name="**Weaknesses:**", value=("{weakness_list}").format(weakness_list=weakness_to_str(channel.guild, get_weaknesses(entered_raid))), inline=True)
        raid_embed.set_footer(text=("Reported by @{author}").format(author=raid_messageauthor.display_name), icon_url=("https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.{format}?size={size}".format(user=raid_messageauthor, format="jpg", size=32)))
        raid_embed.set_thumbnail(url=raid_img_url)
        try:
...
Please type `!beep raid` if you need a refresher of Clembot commands! 
    """).format(member=raid_messageauthor.mention, citychannel=reportcitychannel.mention, pokemon=raid_role, location_details=egg_address)
        ...
        try:
            if eggdetails.get('egglevel', None):
                await channel.send( content=("Beep Beep! Trainers {trainer_list}: The raid egg has just hatched into a {pokemon} raid!").format(trainer_list=", ".join(trainer_list), pokemon=raid_role), embed=raid_embed)
        except Exception as error:
            logger.info(error)
...
def get_help_embed(description, usage, available_value_title, available_values, mode="message"):
    ...
    help_embed.add_field(name="**{0} :**".format(available_value_title), value=("**{0}**".format(", ".join(available_values))), inline=False)
...
async def _importx(ctx, *, gym_list_in_json_text=None):
    try:
...
async def nest(ctx):
    try:
        ...
        nest_embed.set_footer(text=("Reported by @{author}").format(author=message.author.display_name), icon_url=("https://cdn.discordapp.com/avatars/{user.id}/{user.avatar}.{format}?size={size}".format(user=message.author, format="jpg", size=32)))
...
async def gym(ctx, gym_code=None):
    try:
        ...
        message=ctx.message
        parent_city_id = None
        is_raid_channel = check_raid_channel(ctx.message.channel.id)
...
async def _generate_gym_embed_old(message, gym_info):
    try:
        ...
        raid_embed = discord.Embed(title=("Beep Beep! {embed_title}").format(embed_title=embed_title), url=gym_location, description=embed_desription)
        ...
        await message.channel.send( content=("Beep Beep! {member} {roster_message}").format(member=message.author.mention, roster_message=roster_message), embed=raid_embed)
    except Exception as error:
        logger.info(error)
...
def check_raid_channel(channel_id):
    channel = Clembot.get_channel(channel_id)
    type = guild_dict[channel.guild.id]['raidchannel_dict'].get(channel.id,{}).get('type',None)
...
beepmsg = """**{member}, !beep** can be used with following options:
...
@checks.activeraidchannel()
async def omw(ctx):
    await ctx.message.channel.send( content=("Beep Beep! Hey {member}, I don't know if you meant **!coming** to say that you are coming or **!list coming** to see the other trainers on their way").format(member=ctx.message.author.mention))
...
async def location(ctx):
    ...
    if ctx.invoked_subcommand is None:
        ...
        newembed.set_thumbnail(url=oldembed['thumbnail']['url'])
        locationmsg = await channel.send( content=("Beep Beep! Here's the current location for the raid!\nDetails:{location}").format(location=location), embed=newembed)
...
async def update(ctx):
    try:
        ...
        if len(roster) <= 0:
            await ctx.message.channel.send( content=("Beep Beep! The roster doesn't have any location(s)! Type `!beep raidparty` to see how you can manage raid party!"))
            return
        ...
        if location_number == 0:
            await ctx.message.channel.send( content=("Beep Beep! I couldn't understand the location #."))
            return
        ...
        if roster_loc is None:
            await ctx.message.channel.send( content=("Beep Beep! Location {location} doesn't exist on the roster!".format(location=emojify_numbers(location_number))))
            return

        # if len(args_split) > 1:
        #     await ctx.message.channel.send( content=("Beep Beep! That's too much to update... use `!update <location#> <pokemon-name or gym-code or google map link>`"))
        #     return
        ...
        if gym_info:
            ...
            if gmap_link:
                ...
                if eta:
                    ...
                    await ctx.message.channel.send( content=("Beep Beep! I am not sure what to update;... use `!update <location#> <pokemon-name | gym-code | google map link | eta>` "))
                    return
        ...
        await ctx.message.channel.send( content=("Beep Beep! Error : {error} {error_details}").format(error=error, error_details=str(error)))
...
async def _add(message, gmap_link):
    ...
    try:
        ...
        await message.channel.send( content=("Beep Beep! Error : {error} {error_details}").format(error=error, error_details=str(error)))
    return
...
async def _remove(ctx):
    ...
    if is_location_found:
        if len(roster) == 0:
            ...
            await ctx.message.channel.send( content=("Beep Beep! {member}, {roster_message}").format(member=ctx.message.author.mention, roster_message=roster_message))
        else:
...
async def move(ctx):
    ...
    if len(roster) == 0:
        guild_dict[ctx.message.guild.id]['raidchannel_dict'][ctx.message.channel.id]['roster_index'] = first_roster_index
        await ctx.message.channel.send( content=("Beep Beep! {member}, all the locations on this roster are done!").format(member=ctx.message.author.mention))
    else:
...
async def makeitraidparty(ctx):
    ...
    await message.channel.send( content=("Beep Beep! It's a raid party channel now!"))
...
async def reset(ctx):
    ...
    await message.channel.send( content=("Beep Beep! The roster has been cleared!"))
    ...
    try:
        ...
        if len(args_split) > 0:
            ...
            if pathshare_url == None:
                await ctx.message.channel.send( content=("Beep Beep! Give more details! Usage: `!where <location #>`"))
            else:
                await ctx.message.channel.send(content=("Beep Beep! <@!{raidcreator}>'s live location can be accessed at : {pathshare}".format(raidcreator=pathshare_user, pathshare=pathshare_url)))
            return
        ...
        for roster_loc_at in roster:
            if roster_loc_at['index'] == location_number:
                ...
                return
        await ctx.message.channel.send( content=("Beep Beep! The roster doesn't have location {location_number}.".format(location_number=emojify_numbers(location_number))))
        return
...
async def _next_location(ctx):
    ...
    if len(roster) < 2:
        ...
        await ctx.message.channel.send( content=("Beep Beep! {status_message}").format(status_message=status_message))
        return
...
async def print_roster_with_highlight(message, highlight_roster_loc, roster_message=None):
    try:
        ...
        if highlight_roster_loc:
            ...
            if len(roster) < 1:
                await message.channel.send( content=("Beep Beep! The roster doesn't have any location(s)! Type `!beep raidparty` to see how you can manage raid party!"))
                return
        ...
        raid_embed = discord.Embed(title=("Beep Beep! {embed_title}").format(embed_title=embed_title), url=roster_loc_gmap_link, image=raid_img_url, description=embed_desription)
        raid_embed.add_field(name="**Raid Boss:**", value=("{raidboss}").format(raidboss=roster_loc['pokemon'].capitalize()), inline=True)
        raid_embed.add_field(name="**Location Details:**", value=("{location}").format(location=roster_loc['gym_name']), inline=True)
        if 'eta' in roster_loc:
            if roster_loc['eta']:
                raid_embed.add_field(name="**ETA:**", value=("{eta}").format(eta=roster_loc['eta']), inline=True)
        raid_embed.set_thumbnail(url=raid_img_url)
        raid_embed.set_footer(text=("Reported by @{author}").format(author=message.author.display_name), icon_url=message.author.avatar_url)
        if lat_long:
...
async def print_roster(message, roster_message=None):
    ...
    raid_embed = discord.Embed(title=("Beep Beep! {embed_title}").format(embed_title=embed_title), url=raid_party_image_url, description=roster_msg)
    raid_embed.set_footer(text=("Requested by @{author}").format(author=message.author.display_name), icon_url=message.author.avatar_url)
    raid_embed.set_thumbnail(url=raid_img_url)

    if roster_message:
        await message.channel.send( content=("Beep Beep! {member} {roster_message}").format(member=message.author.mention, roster_message=roster_message), embed=raid_embed)
    else:
        await message.channel.send( content=("Beep Beep! {member} here is the raid party roster: ").format(member=message.author.mention), embed=raid_embed)
...
async def _generate_gym_embed(message, gym_info):
    ...
    raid_embed = discord.Embed(title=("Beep Beep! {embed_title}").format(embed_title=embed_title), url=gym_info['gmap_url'], description=embed_desription)
...
async def reloadconfig(ctx):
    try:
        load_config()
        await ctx.message.channel.send( content=("Beep Beep! configuration reloaded!"))
    except Exception as error:
        await ctx.message.channel.send( content=("Beep Beep! Error : {error}").format(error=str(error)))
    return
...
async def _get_gym(ctx):
    ...
    try:
        ...
        if len(args) < 1:
            await ctx.message.channel.send(content=("Beep Beep! Please provide information as !get-gym gym-code"))
...
async def _bingo_win(ctx):
    try:
        ...
        if existing_bingo_card_record:
            raid_embed = discord.Embed(title=("**{0} Shoutout!**".format(event_title_map.get(event_pokemon,"BingO"))), description="", colour=discord.Colour.dark_gold())

            raid_embed.add_field(name="**Member:**", value=("**{member}** called **bingo** at **{timestamp}**.").format(member=message.author.display_name, timestamp=timestamp), inline=True)
...
async def leaderboard(ctx, lb_type="lifetime" , r_type="total"):
    ...
    try:
        ...
        embed = discord.Embed(colour=ctx.guild.me.colour)
        embed.set_author(name=("Leaderboard Type: {leaderboard_type} ({report_type})").format(leaderboard_type=leaderboard_type.title(), report_type=report_type.title()), icon_url=Clembot.user.avatar_url)
...
# try:
#     event_loop.run_until_complete(initializae_dbInterface())
# except Exception as error:
#     print(error)
