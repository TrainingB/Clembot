...
async def on_server_join(server):
    owner = server.owner
    server_dict[server] = {'want_channel_list': [], 'offset': 0, 'welcome': False, 'team': False, 'want': False, 'other': False, 'done': False, 'raidchannel_dict' : {}}
    await Meowth.send_message(owner, _("Beep Beep! I'm Meowth, a Discord helper bot for Pokemon Go communities, and someone has invited me to your server! Type !help to see a list of things I can do, and type !configure in any channel of your server to begin!"))
...
async def configure(ctx):
    ...
    if server_dict_temp['team'] == True:
        await Meowth.send_message(owner, _("Beep Beep! Welcome to {server_name}, {owner_name.mention}! Set your team by typing '!team mystic' or '!team valor' or '!team instinct' without quotations. If you have any questions just ask an admin.").format(server_name=server.name, owner_name=owner))
    else:
        await Meowth.send_message(owner, _("Beep Beep! Welcome to {server_name}, {owner_name.mention}! If you have any questions just ask an admin.").format(server_name=server, owner_name=owner))
                    ...
                    if not diff:
                        ...
                        while True:
                            try:
                                ...
                                await Meowth.send_message(owner, _("Beep Beep! You didn't give me enough permissions to create channels! Please check my permissions and that my role is above general roles. Let me know if you'd like me to check again.\n\nRespond with: **Y** to try again, or **N** to skip and create the missing channels yourself."))
                                while True:
    ...
    await Meowth.send_message(owner, _("Beep Beep! Alright! Your settings have been saved and I'm ready to go! If you need to change any of these settings, just type !configure in your server again."))
...
async def team(ctx):
    ...
    if high_roles:
        await Meowth.send_message(ctx.message.channel, _("Beep Beep! My roles are ranked lower than the following team roles: **{higher_roles_list}**\nPlease get an admin to move my roles above them!").format(higher_roles_list=', '.join(high_roles)))
        return
    ...
    for team in config['team_dict'].keys():
        ...
        if temp_role:
            ...
            if temp_role in ctx.message.author.roles:
                ...
                await Meowth.send_message(ctx.message.channel, _("Beep Beep! You already have a team role!"))
                return
    ...
    if entered_team not in config['team_dict'].keys():
        await Meowth.send_message(ctx.message.channel, _("Beep Beep! \"{entered_team}\" isn't a valid team! Try {available_teams}").format(entered_team=entered_team, available_teams=team_msg))
        return
    # Check if the role is configured on the server
    elif role is None:
        await Meowth.send_message(ctx.message.channel, _("Beep Beep! The \"{entered_team}\" role isn't configured on this server! Contact an admin!").format(entered_team=entered_team))
    else:
        try:
            await Meowth.add_roles(ctx.message.author, role)
            await Meowth.send_message(ctx.message.channel, _("Beep Beep! Added {member} to Team {team_name}! {team_emoji}").format(member=ctx.message.author.mention, team_name=role.name.capitalize(), team_emoji=config['team_dict'][entered_team]))
        except discord.Forbidden:
            await Meowth.send_message(ctx.message.channel, _("Beep Beep! I can't add roles!"))
...
async def want(ctx):
    ...
    if server_dict[ctx.message.server]['wantset'] == True:
        if server_dict[ctx.message.server]['want_channel_list'] and ctx.message.channel not in server_dict[ctx.message.server]['want_channel_list']:
            await Meowth.send_message(ctx.message.channel, _("Beep Beep! Please use one of the following channels for **!want** commands: {want_channel_list}").format(want_channel_list=", ".join(i.mention for i in server_dict[ctx.message.server]['want_channel_list'])))
            return
        else:
            ...
            if role in ctx.message.author.roles:
                await Meowth.send_message(ctx.message.channel, content=_("Beep Beep! {member}, I already know you want {pokemon}!").format(member=ctx.message.author.mention, pokemon=entered_want.capitalize()))
            else:
                await Meowth.add_roles(ctx.message.author, role)
                ...
                await Meowth.send_message(ctx.message.channel, content=_("Beep Beep! Got it! {member} wants {pokemon}").format(member=ctx.message.author.mention, pokemon=entered_want.capitalize()),embed=want_embed)
...
async def _wild(message):
    if server_dict[message.server]['wildset'] == True:
        if message.channel.name not in server_dict[message.server]['city_channels'].keys() and message.channel != message.server.default_channel:
            await Meowth.send_message(message.channel, _("Beep Beep! Please restrict wild reports to city channels or the default channel!"))
            return
        space1 = message.content.find(" ",6)
        if space1 == -1:
            await Meowth.send_message(message.channel, _("Beep Beep! Give more details when reporting! Usage: **!wild <pokemon name> <location>**"))
            return
        else:
        ...
        if entered_wild not in pkmn_info['pokemon_list']:
            ...
            await Meowth.send_message(message.channel, content=_("Beep Beep! Wild {pokemon} reported by {member}! Details: {location_details}").format(pokemon=wild.mention, member=message.author.mention, location_details=wild_details),embed=wild_embed)
    else:
        await Meowth.send_message(message.channel, _("Beep Beep! **!wild** commands have been disabled."))
    ...
    if server_dict[message.server]['raidset'] == True:
        ...
        if message.channel.name not in server_dict[message.server]['city_channels'].keys():
            if message.channel in server_dict[message.channel.server]['raidchannel_dict'] and server_dict[message.channel.server]['raidchannel_dict'][message.channel]['type'] == 'egg':
                ...
                await Meowth.send_message(message.channel, _("Beep Beep! Please restrict raid reports to a city channel!"))
                return
        ...
        if fromegg is True:
            if args_split[0] == 'assume':
                ...
                if server_dict[message.channel.server]['raidchannel_dict'][message.channel]['active'] == False:
                    ...
                    await Meowth.send_message(message.channel, _("Beep Beep! Please wait until the egg has hatched before changing it to an open raid!"))
                    return
        elif len(args_split) <= 1:
            await Meowth.send_message(message.channel, _("Beep Beep! Give more details when reporting! Usage: **!raid <pokemon name> <location>**"))
            return
        ...
        if entered_raid not in pkmn_info['raid_list'] and entered_raid in pkmn_info['pokemon_list']:
            await Meowth.send_message(message.channel, _("Beep Beep! The Pokemon {pokemon} does not appear in raids!").format(pokemon=entered_raid.capitalize()))
            return
        ...
        raidreport = await Meowth.send_message(message.channel, content = _("Beep Beep! {pokemon} raid reported by {member}! Details: {location_details}. Coordinate in {raid_channel}").format(pokemon=entered_raid.capitalize(), member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention),embed=raid_embed)
...
This channel will be deleted five minutes after the timer expires.""").format(pokemon=raid.mention, member=message.author.mention, citychannel=message.channel.mention, location_details=raid_details)
        ...
        if raidexp:
            ...
            await Meowth.send_message(raid_channel, content = _("Beep Beep! Hey {member}, if you can, set the time left on the raid using **!timerset <minutes>** so others can check it with **!timer**.").format(member=message.author.mention))

        event_loop.create_task(expiry_check(raid_channel))
    else:
        await Meowth.send_message(message.channel, _("Beep Beep! **!raid** commands have been disabled."))
...
async def unwant(ctx):
    ...
    if server_dict[ctx.message.server]['wantset'] == True:
        if server_dict[ctx.message.server]['want_channel_list'] and ctx.message.channel not in server_dict[ctx.message.server]['want_channel_list']:
            await Meowth.send_message(ctx.message.channel, _("Beep Beep! Please use one of the following channels for **!unwant** commands: {want_channel_list}").format(want_channel_list=", ".join(i.mention for i in server_dict[ctx.message.server]['want_channel_list'])))
            return
        else:
            ...
            if entered_unwant not in pkmn_info['pokemon_list']:
                ...
                if role not in ctx.message.author.roles:
                    ...
                    #await Meowth.send_message(ctx.message.channel, content=_("Beep Beep! {member}, I already know you don't want {pokemon}!").format(member=ctx.message.author.mention, pokemon=entered_unwant.capitalize()))
                else:
                    ...
                    #await Meowth.send_message(ctx.message.channel, content=_("Beep Beep! Got it! {member} no longer wants {pokemon}").format(member=ctx.message.author.mention, pokemon=entered_unwant.capitalize()),embed=unwant_embed)
...
async def print_raid_timer(channel):
    ...
    if not server_dict[channel.server]['raidchannel_dict'][channel]['active']:
        timerstr += _("Beep Beep! This {raidtype}'s timer has already expired as of {expiry_time}!").format(raidtype=raidtype,expiry_time=strftime("%I:%M", localexpire))
    else:
        if server_dict[channel.server]['raidchannel_dict'][channel]['manual_timer']:
            timerstr += _("Beep Beep! This {raidtype} will {raidaction} at {expiry_time}!").format(raidtype=raidtype,raidaction=raidaction,expiry_time=strftime("%I:%M", localexpire))
        else:
            timerstr += _("Beep Beep! No one told me when the {raidtype} will {raidaction}, so I'm assuming it will {raidaction} at {expiry_time}!").format(raidtype=raidtype,raidaction=raidaction,expiry_time=strftime("%I:%M", localexpire))
...
async def _timerset(channel, exptime):
    ...
    if channel in server_dict[channel.server]['raidchannel_dict']:
        ...
        try:
            s = exptime * 60
            if s >= 3600:
                await Meowth.send_message(channel, _("Beep Beep...that's too long. {raidtype} currently last no more than one hour...").format(raidtype=raidtype))
                return
            if s < 0:
                await Meowth.send_message(channel, _("Beep Beep...I can't do that! No negative numbers, please!"))
                return
        except:
            await Meowth.send_message(channel, _("Beep Beep...I couldn't understand your time format..."))
            return
        expire = ticks + s
...
async def timerset(ctx):
    ...
    if ctx.message.channel in server_dict[ctx.message.server]['raidchannel_dict']:
        ...
        if str(exptime).isdigit():
            ...
            await Meowth.send_message(ctx.message.channel, _("Beep Beep... I couldn't understand your time format. Try again like this: !timerset <minutes>"))
...
async def _maybe(message, count):
    if message.channel in server_dict[message.server]['raidchannel_dict'] and server_dict[message.server]['raidchannel_dict'][message.channel]['active']:
        ...
        if count == 1:
            await Meowth.send_message(message.channel, _("Beep Beep! {member} is interested!").format(member=message.author.mention))
        else:
            await Meowth.send_message(message.channel, _("Beep Beep! {member} is interested with a total of {trainer_count} trainers!").format(member=message.author.mention, trainer_count=count))
...
async def _coming(message, count):
    if message.channel in server_dict[message.server]['raidchannel_dict'] and server_dict[message.server]['raidchannel_dict'][message.channel]['active']:
        ...
        if count == 1:
            await Meowth.send_message(message.channel, _("Beep Beep! {member} is on the way!").format(member=message.author.mention))
        else:
            await Meowth.send_message(message.channel, _("Beep Beep! {member} is on the way with a total of {trainer_count} trainers!").format(member=message.author.mention, trainer_count=count))
        # Add trainer name to trainer list
...
async def _here(message, count):
    if message.channel in server_dict[message.server]['raidchannel_dict'] and server_dict[message.server]['raidchannel_dict'][message.channel]['active']:
        trainer_dict = server_dict[message.server]['raidchannel_dict'][message.channel]['trainer_dict']
        if count == 1:
            await Meowth.send_message(message.channel, _("Beep Beep! {member} is at the raid!").format(member=message.author.mention))
        else:
            await Meowth.send_message(message.channel, _("Beep Beep! {member} is at the raid with a total of {trainer_count} trainers!").format(member=message.author.mention, trainer_count=count))
        # Add trainer name to trainer list
        if message.author.mention not in trainer_dict:
            trainer_dict[message.author.mention] = {}
    ...
    if message.channel in server_dict[message.server]['raidchannel_dict'] and server_dict[message.server]['raidchannel_dict'][message.channel]['active']:
        ...
        if message.author.mention in trainer_dict and trainer_dict[message.author.mention]['status'] == "maybe":
            if trainer_dict[message.author.mention]['count'] == 1:
                await Meowth.send_message(message.channel, _("Beep Beep! {member} is no longer interested!").format(member=message.author.mention))
            else:
                await Meowth.send_message(message.channel, _("Beep Beep! {member} and their total of {trainer_count} trainers are no longer interested!").format(member=message.author.mention, trainer_count=trainer_dict[message.author.mention]['count']))
        if message.author.mention in trainer_dict and trainer_dict[message.author.mention]['status'] == "waiting":
            if trainer_dict[message.author.mention]['count'] == 1:
                await Meowth.send_message(message.channel, _("Beep Beep! {member} has left the raid!").format(member=message.author.mention))
            else:
                await Meowth.send_message(message.channel, _("Beep Beep! {member} and their total of {trainer_count} trainers have left the raid!").format(member=message.author.mention, trainer_count=trainer_dict[message.author.mention]['count']))
        if message.author.mention in trainer_dict and trainer_dict[message.author.mention]['status'] == "omw":
            if trainer_dict[message.author.mention]['count'] == 1:
                await Meowth.send_message(message.channel, _("Beep Beep! {member} is no longer on their way!").format(member=message.author.mention))
            else:
                await Meowth.send_message(message.channel, _("Beep Beep! {member} and their total of {trainer_count} trainers are no longer on their way!").format(member=message.author.mention, trainer_count=trainer_dict[message.author.mention]['count']))
    ...
    if ctx.message.channel in server_dict[ctx.message.server]['raidchannel_dict'] and server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['active']:
        ...
        if count:
            if count.isdigit():
                ...
                await Meowth.send_message(ctx.message.channel, _("Beep Beep! I can't understand how many are in your group. Just say `!interested` if you're by yourself, or `!interested 5` for example if there's 5 in your group."))
        else:
    ...
    if ctx.message.channel in server_dict[ctx.message.server]['raidchannel_dict'] and server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['active']:
        try:
            if server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['type'] == "egg":
                if server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['pokemon'] == "":
                    await Meowth.send_message(ctx.message.channel, _("Beep Beep! Please wait until the raid egg has hatched before announcing you're coming or present."))
                    return
        except:
            pass
        ...
        if count:
            if count.isdigit():
                ...
                await Meowth.send_message(ctx.message.channel, _("Beep Beep! I can't understand how many are in your group. Just say `!coming` if you're by yourself, or `!coming 5` for example if there's 5 in your group."))
                return
        else:
            if ctx.message.author.mention in trainer_dict:
                count = trainer_dict[ctx.message.author.mention]['count']
    ...
    if ctx.message.channel in server_dict[ctx.message.server]['raidchannel_dict'] and server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['active']:
        ...
        if count:
            if count.isdigit():
                ...
                await Meowth.send_message(ctx.message.channel, _("Beep Beep! I can't understand how many are in your group. Just say `!here` if you're by yourself, or `!coming 5` for example if there's 5 in your group."))
        else:
            if ctx.message.author.mention in trainer_dict:
                count = trainer_dict[ctx.message.author.mention]['count']
            else:
...
async def omw(ctx):
    if ctx.message.channel in server_dict[ctx.message.server]['raidchannel_dict'] and server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['active']:
        await Meowth.send_message(ctx.message.channel, content = _("Beep Beep! Hey {member}, I don't know if you meant **!coming** to say that you are coming or **!otw** to see the other trainers on their way").format(member=ctx.message.author.mention))
        ...
        if raid_status is not None:
            if server_dict[message.server]['raidchannel_dict'][message.channel]['active']:
                ...
                if "/maps" in message.content:
                    ...
                    await Meowth.send_message(message.channel, content = _("Beep Beep! Someone has suggested a different location for the raid! Trainers {trainer_list}: make sure you are headed to the right place!").format(trainer_list=", ".join(otw_list)), embed = newembed)
                    return
...
async def duplicate(ctx):
    ...
    if channel in server_dict[server]['raidchannel_dict'] and server_dict[server]['raidchannel_dict'][channel]['active']:
            ...
            if author in trainer_dict:
                try:
                    if trainer_dict[author]['dupereporter']:
                        dupeauthmsg = await Meowth.send_message(channel,_("Beep Beep! You've already made a duplicate report for this raid!"))
        ...
        if dupecount >= 3:
            rusure = await Meowth.send_message(channel,_("Beep Beep! Are you sure you wish to remove this raid?"))
...
async def location(ctx):
    ...
    if ctx.invoked_subcommand is None:
        if ctx.message.channel in server_dict[ctx.message.server]['raidchannel_dict'] and server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['active']:
            ...
            newembed.set_thumbnail(url=oldembed['thumbnail']['url'])
            await Meowth.send_message(message.channel, content = _("Beep Beep! Here's the current location for the raid!\nDetails:{location}").format(location = location), embed = newembed)
...
async def new(ctx):
    ...
    if ctx.message.channel in server_dict[ctx.message.server]['raidchannel_dict'] and server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['active']:
        ...
        if space1 == -1:
            await Meowth.send_message(message.channel, _("Beep Beep! We're missing the new location details! Usage: **!location new <new address>**"))
            return
        else:
            report_city = server_dict[message.server]['raidchannel_dict'][message.channel]['reportcity']
            report_channel = discord.utils.get(message.server.channels, name=report_city)
...
async def _exraid(message):
    if server_dict[message.server]['raidset'] == True:
        ...
        del args_split[0]
        if len(args_split) <= 1:
            await Meowth.send_message(message.channel, _("Beep Beep! Give more details when reporting! Usage: **!exraid <pokemon name> <location>**"))
            return
        entered_raid = re.sub("[\@]", "", args_split[0].lower())
        del args_split[0]
        if entered_raid not in pkmn_info['pokemon_list']:
        ...
        raid_embed.set_thumbnail(url=raid_img_url)
        raidreport = await Meowth.send_message(message.channel, content = _("Beep Beep! {pokemon} EX raid reported by {member}! Details: {location_details}. Send proof of your invite to this EX raid to an admin and coordinate in {raid_channel}").format(pokemon=entered_raid.capitalize(), member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention),embed=raid_embed)
        await asyncio.sleep(1) #Wait for the channel to be created.

        raidmsg = _("""Beep Beep! {pokemon} EX raid reported by {member}! Details: {location_details}. Coordinate here!
...
This channel will be deleted in 2 days if an admin doesn't delete it manually before then!""").format(pokemon=raid.mention, member=message.author.mention, location_details=raid_details)
        ...
        await Meowth.send_message(message.channel, _("Beep Beep! **!exraid** commands have been disabled."))
...
async def _raidegg(message):
    if server_dict[message.server]['raidset'] == True:
        ...
        del args_split[0]
        if len(args_split) <= 1:
            await Meowth.send_message(message.channel, _("Beep Beep! Give more details when reporting! Usage: **!raidegg <level> <location>**"))
            return
        ...
        if args_split[0].isdigit():
            ...
            await Meowth.send_message(message.channel, _("Beep Beep! Give more details when reporting! Use at least: **!raidegg <level> <location>**. Type !help raidegg for more info."))
            return
        ...
        if egg_level > 5 or egg_level == 0:
            await Meowth.send_message(message.channel, _("Beep Beep! Raid egg levels are only from 1-5!"))
            return
        else:
            ...
            raid_embed.set_thumbnail(url=raid_img_url)
            raidreport = await Meowth.send_message(message.channel, content = _("Beep Beep! Level {level} raid egg reported by {member}! Details: {location_details}. Coordinate in {raid_channel}").format(level=egg_level, member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention),embed=raid_embed)
            await asyncio.sleep(1) #Wait for the channel to be created.
...
async def _eggassume(args, raid_channel):
    ...
    if config['allow_assume'][egglevel] == "False":
        await Meowth.send_message(raid_channel, _("Beep Beep! **!raid assume** is not allowed in this level egg."))
        return
    ...
    if entered_raid not in pkmn_info['pokemon_list']:
        ...
        if entered_raid not in pkmn_info['raid_list']:
            await Meowth.send_message(raid_channel, _("Beep Beep! The Pokemon {pokemon} does not appear in raids!").format(pokemon=entered_raid.capitalize()))
            return
        else:
            if get_number(entered_raid) not in raid_info['raid_eggs'][egglevel]['pokemon']:
                await Meowth.send_message(raid_channel, _("Beep Beep! The Pokemon {pokemon} does not hatch from level {level} raid eggs!").format(pokemon=entered_raid.capitalize(), level=egglevel))
                return
    ...
    await Meowth.send_message(raid_channel, _("Beep Beep! This egg will be assumed to be {pokemon} when it hatches!").format(pokemon=raidrole.mention))
    return
...
async def _eggtoraid(entered_raid, raid_channel):
    ...
    raidmsg = _("""Beep Beep! The egg hatched into a {pokemon} raid! Details: {location_details}. Coordinate here!
                ...
                if ctx.message.channel.name in server_dict[ctx.message.server]['city_channels'].keys():
                    ...
                    for activeraid in server_dict[ctx.message.server]['raidchannel_dict']:
                        ...
                        if server_dict[ctx.message.server]['raidchannel_dict'][activeraid]['reportcity'] == ctx.message.channel.name and server_dict[ctx.message.server]['raidchannel_dict'][activeraid]['active'] and discord.utils.get(ctx.message.channel.server.channels, id=activeraid.id):
                            ...
                            activeraidnum += 1
                    if activeraidnum == 0:
                        await Meowth.send_message(ctx.message.channel, _("Beep Beep! No active raids! Report one with **!raid <name> <location>**."))
                        return
...
async def interest(ctx):
    await Meowth.send_message(ctx.message.channel, _("Beep Beep! We've moved this command to **!list interested**."))

@Meowth.command(pass_context=True, hidden=True)
async def otw(ctx):
    await Meowth.send_message(ctx.message.channel, _("Beep Beep! We've moved this command to **!list coming**."))

@Meowth.command(pass_context=True, hidden=True)
async def waiting(ctx):
    await Meowth.send_message(ctx.message.channel, _("Beep Beep! We've moved this command to **!list here**."))
        ...
        if 'https://cdn.discordapp.com' in ctx.message.attachments[0]['url']:
            if 'png' in ctx.message.attachments[0]['url'].lower() or 'jpg' in ctx.message.attachments[0]['url'].lower():
                ...
                if 'EX Raid Battle' or "This is a reward" or "Please visit the Gym" in txt:
                    ...
                    if exraidcount > 0:
                        await Meowth.send_message(ctx.message.channel, "Beep Beep! {0}, it looks like you've got an EX Raid invitation! The following {1} EX Raids have been reported: \n {2} \n Reply with the number of the EX Raid you have been invited to. If none of them match your invite, type 'N' and report it with !exraid".format(ctx.message.author.mention, str(exraidcount), exraidlist))
                        reply = await Meowth.wait_for_message(author=ctx.message.author)
                        if reply.content.lower() == 'n':
                            await Meowth.send_message(ctx.message.channel, "Beep Beep! Be sure to report your EX Raid with `!exraid`!")
                        elif not reply.content.isdigit() or int(reply.content) > exraidcount:
                            await Meowth.send_message(ctx.message.channel, "Beep Beep! I couldn't tell which EX Raid you meant! Try the `!invite` command again, and make sure you respond with the number of the channel that matches!")
                            ...
                            await Meowth.send_message(ctx.message.channel, "Beep Beep! Alright {0}, you can now send messages in {1}! Make sure you let the trainers in there know if you can make it to the EX Raid!".format(ctx.message.author.mention, exraid_channel.mention))
                        else:
                            await Meowth.send_message(ctx.message.channel, "Beep Beep! I couldn't understand your reply! Try the `!invite` command again!")
                    else:
                        await Meowth.send_message(ctx.message.channel, "Beep Beep! No EX Raids have been reported in this server! Use `!exraid` to report one!")
                else:
                    await Meowth.send_message(ctx.message.channel, "Beep Beep! That doesn't look like an EX Raid invitation to me! If it is, please message an admin to get added to the EX Raid channel manually!")
            else:
                await Meowth.send_message(ctx.message.channel, "Beep Beep! Your attachment was not a supported image format!")
        else:
            await Meowth.send_message(ctx.message.channel, "Beep Beep! Please upload your screenshot directly to Discord!")
    else:
        await Meowth.send_message(ctx.message.channel, "Beep Beep! You need to attach a screenshot of your invite to the message!")
