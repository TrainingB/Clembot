...
def spellcheck(word):
    ...
    if suggestion != word:
        return _("Clembot! \"{entered_word}\" is not a Pokemon! Did you mean \"{corrected_word}\"?").format(entered_word=word, corrected_word=spelling.correction(word))
    else:
        return _("Clembot! \"{entered_word}\" is not a Pokemon! Check your spelling!").format(entered_word=word)
...
async def reboot_msg(owners,loop=False,):
    msg_success = 0
    msg_fail = 0
    reboot_msg = """**Clembot! That's right! I've been updated!**
...
async def on_server_join(server):
    owner = server.owner
    server_dict[server] = {'want_channel_list': [], 'offset': 0, 'welcome': False, 'team': False, 'want': False, 'other': False, 'done': False, 'raidchannel_dict' : {}}
    await Meowth.send_message(owner, _("Clembot! I'm Clembot, a Discord helper bot for Pokemon Go communities, and someone has invited me to your server! Type **!help** to see a list of things I can do, and type **!configure** in any channel of your server to begin!"))
...
async def configure(ctx):
    ...
    if server_dict_temp['team'] == True:
        await Meowth.send_message(owner, _("Clembot! Welcome to {server_name}, {owner_name.mention}! Set your team by typing '**!team mystic**' or '**!team valor**' or '**!team instinct**' without quotations. If you have any questions just ask an admin.").format(server_name=server.name, owner_name=owner))
    else:
        await Meowth.send_message(owner, _("Clembot! Welcome to {server_name}, {owner_name.mention}! If you have any questions just ask an admin.").format(server_name=server, owner_name=owner))
                    ...
                    if not diff:
                        ...
                        while True:
                            try:
                                ...
                                await Meowth.send_message(owner, _("Clembot! You didn't give me enough permissions to create channels! Please check my permissions and that my role is above general roles. Let me know if you'd like me to check again.\n\nRespond with: **Y** to try again, or **N** to skip and create the missing channels yourself."))
                                while True:
    ...
    await Meowth.send_message(owner, _("Clembot! Alright! Your settings have been saved and I'm ready to go! If you need to change any of these settings, just type **!configure** in your server again."))
...
async def on_member_join(member):
    ...
    welcomemessage = _("Clembot! Welcome to {server_name}, {new_member_name}! ")
...
async def team(ctx):
    ...
    if high_roles:
        await Meowth.send_message(ctx.message.channel, _("Clembot! My roles are ranked lower than the following team roles: **{higher_roles_list}**\nPlease get an admin to move my roles above them!").format(higher_roles_list=', '.join(high_roles)))
        return
    ...
    for team in config['team_dict'].keys():
        ...
        if temp_role:
            ...
            if temp_role in ctx.message.author.roles:
                ...
                await Meowth.send_message(ctx.message.channel, _("Clembot! You already have a team role!"))
                return
    ...
    if entered_team not in config['team_dict'].keys():
        await Meowth.send_message(ctx.message.channel, _("Clembot! \"{entered_team}\" isn't a valid team! Try {available_teams}").format(entered_team=entered_team, available_teams=team_msg))
        return
    # Check if the role is configured on the server
    elif role is None:
        await Meowth.send_message(ctx.message.channel, _("Clembot! The \"{entered_team}\" role isn't configured on this server! Contact an admin!").format(entered_team=entered_team))
    else:
        try:
            await Meowth.add_roles(ctx.message.author, role)
            await Meowth.send_message(ctx.message.channel, _("Clembot! Added {member} to Team {team_name}! {team_emoji}").format(member=ctx.message.author.mention, team_name=role.name.capitalize(), team_emoji=config['team_dict'][entered_team]))
        except discord.Forbidden:
            await Meowth.send_message(ctx.message.channel, _("Clembot! I can't add roles!"))
...
async def want(ctx):
    ...
    if checks.check_wantchannel(ctx):
        ...
        if role in ctx.message.author.roles:
            await Meowth.send_message(channel, content=_("Clembot! {member}, I already know you want {pokemon}!").format(member=ctx.message.author.mention, pokemon=entered_want.capitalize()))
        else:
            await Meowth.add_roles(ctx.message.author, role)
            ...
            await Meowth.send_message(channel, content=_("Clembot! Got it! {member} wants {pokemon}").format(member=ctx.message.author.mention, pokemon=entered_want.capitalize()),embed=want_embed)
    else:
        want_channels = server_dict[server]['want_channel_list']
        await ctx.bot.send_message(channel, "Clembot! Please use one of the following channels for **!want** commands: {want_channel_list}".format(want_channel_list=", ".join(c.mention for c in want_channels)))
...
async def unwant(ctx):
    ...
    if ctx.invoked_subcommand is None:
        if checks.check_wantchannel(ctx):
            ...
            want_channels = server_dict[server]['want_channel_list']
            await ctx.bot.send_message(channel, "Clembot! Please use one of the following channels for **!unwant** commands: {want_channel_list}".format(want_channel_list=", ".join(c.mention for c in want_channels)))
...
async def _wild(message):
    ...
    if space1 == -1:
        await Meowth.send_message(message.channel, _("Clembot! Give more details when reporting! Usage: **!wild <pokemon name> <location>**"))
        return
    else:
    ...
    if entered_wild not in pkmn_info['pokemon_list']:
        ...
        wild_embed = discord.Embed(title=_("Clembot! Click here for directions to the wild {pokemon}!").format(pokemon=entered_wild.capitalize()),url=wild_gmaps_link,description=_("This is just my best guess!"),colour=discord.Colour(0x2ecc71))
        wild_embed.set_thumbnail(url=wild_img_url)
        await Meowth.send_message(message.channel, content=_("Clembot! Wild {pokemon} reported by {member}! Details: {location_details}").format(pokemon=wild.mention, member=message.author.mention, location_details=wild_details),embed=wild_embed)
...
async def _raid(message):
    ...
    if message.channel.name not in server_dict[message.server]['city_channels'].keys():
        if message.channel in server_dict[message.channel.server]['raidchannel_dict'] and server_dict[message.channel.server]['raidchannel_dict'][message.channel]['type'] == 'egg':
            ...
            await Meowth.send_message(message.channel, _("Clembot! Please restrict raid reports to a city channel!"))
            return
    ...
    if fromegg is True:
        if args_split[0] == 'assume':
            ...
            if server_dict[message.channel.server]['raidchannel_dict'][message.channel]['active'] == False:
                ...
                await Meowth.send_message(message.channel, _("Clembot! Please wait until the egg has hatched before changing it to an open raid!"))
                return
    elif len(args_split) <= 1:
        await Meowth.send_message(message.channel, _("Clembot! Give more details when reporting! Usage: **!raid <pokemon name> <location>**"))
        return
    ...
    if entered_raid not in pkmn_info['raid_list'] and entered_raid in pkmn_info['pokemon_list']:
        await Meowth.send_message(message.channel, _("Clembot! The Pokemon {pokemon} does not appear in raids!").format(pokemon=entered_raid.capitalize()))
        return
    ...
    raid_embed = discord.Embed(title=_("Clembot! Click here for directions to the raid!"),url=raid_gmaps_link,description=_("Weaknesses: {weakness_list}").format(weakness_list=weakness_to_str(message.server, get_weaknesses(entered_raid))),colour=discord.Colour(0x2ecc71))
    raid_embed.set_thumbnail(url=raid_img_url)
    raidreport = await Meowth.send_message(message.channel, content = _("Clembot! {pokemon} raid reported by {member}! Details: {location_details}. Coordinate in {raid_channel}").format(pokemon=entered_raid.capitalize(), member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention),embed=raid_embed)
    await asyncio.sleep(1) #Wait for the channel to be created.

    raidmsg = _("""Clembot! {pokemon} raid reported by {member} in {citychannel}! Details: {location_details}. Coordinate here!
...
This channel will be deleted five minutes after the timer expires.""").format(pokemon=raid.mention, member=message.author.mention, citychannel=message.channel.mention, location_details=raid_details)
    ...
    if raidexp:
        ...
        await Meowth.send_message(raid_channel, content = _("Clembot! Hey {member}, if you can, set the time left on the raid using **!timerset <minutes>** so others can check it with **!timer**.").format(member=message.author.mention))
...
async def print_raid_timer(channel):
    ...
    if not server_dict[channel.server]['raidchannel_dict'][channel]['active']:
        timerstr += _("Clembot! This {raidtype}'s timer has already expired as of {expiry_time} ({expiry_time24})!").format(raidtype=raidtype,expiry_time=strftime("%I:%M %p", localexpire),expiry_time24=strftime("%H:%M", localexpire))
    else:
        if server_dict[channel.server]['raidchannel_dict'][channel]['manual_timer']:
            timerstr += _("Clembot! This {raidtype} will {raidaction} at {expiry_time} ({expiry_time24})!").format(raidtype=raidtype,raidaction=raidaction,expiry_time=strftime("%I:%M %p", localexpire),expiry_time24=strftime("%H:%M", localexpire))
        else:
            timerstr += _("Clembot! No one told me when the {raidtype} will {raidaction}, so I'm assuming it will {raidaction} at {expiry_time} ({expiry_time24})!").format(raidtype=raidtype,raidaction=raidaction,expiry_time=strftime("%I:%M %p", localexpire),expiry_time24=strftime("%H:%M", localexpire))
...
async def _timerset(channel, exptime):
    ...
    try:
        s = exptime * 60
        if s >= 3600:
            await Meowth.send_message(channel, _("Clembot...that's too long. {raidtype} currently last no more than one hour...").format(raidtype=raidtype))
            return
        if s < 0:
            await Meowth.send_message(channel, _("Clembot...I can't do that! No negative numbers, please!"))
            return
    except:
        await Meowth.send_message(channel, _("Clembot...I couldn't understand your time format..."))
        return
    expire = ticks + s
...
async def timerset(ctx):
    ...
    if args.isdigit():
        ...
        if h.isdigit() and m.isdigit():
            ...
            await Meowth.send_message(ctx.message.channel, "Clembot! I couldn't understand your time format. Try again like this: **!timerset <minutes>**")
            return
    if str(raidexp).isdigit():
        await _timerset(ctx.message.channel, raidexp)
    else:
        await Meowth.send_message(ctx.message.channel, _("Clembot... I couldn't understand your time format. Try again like this: **!timerset <minutes>**"))
...
async def validate_start_time(channel, start_time):
    ...
    if suggested_start_time :
        if suggested_start_time > raid_expires_at:
            await Meowth.send_message(channel, ("Beep Beep...! start time cannot be after raid expiry time!"))
            return None
        elif suggested_start_time < current_datetime:
            if is_raid_egg:
                await Meowth.send_message(channel, ("Beep Beep...! start time cannot be before the egg hatches!"))
            else:
                await Meowth.send_message(channel, ("Beep Beep...! start time cannot be in past!"))
            return None
    else:
        return None
...
async def start(ctx):
    ...
    if ctx.message.channel in server_dict[ctx.message.server]['raidchannel_dict']:
        try:
            if server_dict[ctx.message.channel.server]['raidchannel_dict'][ctx.message.channel]['type'] == 'exraid':
                await Meowth.send_message(ctx.message.channel, _("start isn't supported for exraids."))
                return
        except KeyError:
            pass
        ...
        if start_time is None:
            await Meowth.send_message(ctx.message.channel, _("Beep Beep... I couldn't understand your time format. Try again like this: `!start HH:MM AM/PM`"))
            return
        ...
        if raid_starts_at:
            try:
                ...
                await Meowth.send_message(ctx.message.channel, _("Beep Beep! {member} suggested the start time : {starttime}").format(member=ctx.message.author.mention, starttime=print_24_hour(raid_starts_at)))
            except Exception as error:
                print(error)
            return
...
async def _maybe(message, count):
    ...
    if count == 1:
        await Meowth.send_message(message.channel, _("Clembot! {member} is interested!").format(member=message.author.mention))
    else:
        await Meowth.send_message(message.channel, _("Clembot! {member} is interested with a total of {trainer_count} trainers!").format(member=message.author.mention, trainer_count=count))
...
async def _coming(message, count):
    ...
    if count == 1:
        await Meowth.send_message(message.channel, _("Clembot! {member} is on the way!").format(member=message.author.mention))
    else:
        await Meowth.send_message(message.channel, _("Clembot! {member} is on the way with a total of {trainer_count} trainers!").format(member=message.author.mention, trainer_count=count))
    # Add trainer name to trainer list
...
async def _here(message, count):
    trainer_dict = server_dict[message.server]['raidchannel_dict'][message.channel]['trainer_dict']
    if count == 1:
        await Meowth.send_message(message.channel, _("Clembot! {member} is at the raid!").format(member=message.author.mention))
    else:
        await Meowth.send_message(message.channel, _("Clembot! {member} is at the raid with a total of {trainer_count} trainers!").format(member=message.author.mention, trainer_count=count))
    # Add trainer name to trainer list
    if message.author.mention not in trainer_dict:
        trainer_dict[message.author.mention] = {}
...
async def _cancel(message):
    ...
    try:
        ...
        await Meowth.send_message(channel, _("Clembot! {member} has no status to cancel!").format(member=author.mention))

    if t_dict['status'] == "maybe":
        if t_dict['count'] == 1:
            await Meowth.send_message(channel, _("Clembot! {member} is no longer interested!").format(member=author.mention))
        else:
            await Meowth.send_message(channel, _("Clembot! {member} and their total of {trainer_count} trainers are no longer interested!").format(member=author.mention, trainer_count=t_dict['count']))
    if t_dict['status'] == "waiting":
        if t_dict['count'] == 1:
            await Meowth.send_message(channel, _("Clembot! {member} has left the raid!").format(member=author.mention))
        else:
            await Meowth.send_message(channel, _("Clembot! {member} and their total of {trainer_count} trainers have left the raid!").format(member=author.mention, trainer_count=t_dict['count']))
    if t_dict['status'] == "omw":
        if t_dict['count'] == 1:
            await Meowth.send_message(channel, _("Clembot! {member} is no longer on their way!").format(member=author.mention))
        else:
            await Meowth.send_message(channel, _("Clembot! {member} and their total of {trainer_count} trainers are no longer on their way!").format(member=author.mention, trainer_count=t_dict['count']))
    t_dict['status'] = None
        ...
        if raid_status is not None:
            if server_dict[message.server]['raidchannel_dict'][message.channel]['active']:
                ...
                if "/maps" in message.content:
                    ...
                    await Meowth.send_message(message.channel, content = _("Clembot! Someone has suggested a different location for the raid! Trainers {trainer_list}: make sure you are headed to the right place!").format(trainer_list=", ".join(otw_list)), embed = newembed)
                    return
...
async def _exraid(message):
    ...
    del args_split[0]
    if len(args_split) <= 1:
        await Meowth.send_message(message.channel, _("Clembot! Give more details when reporting! Usage: **!exraid <pokemon name> <location>**"))
        return
    entered_raid = re.sub("[\@]", "", args_split[0].lower())
    del args_split[0]
    if entered_raid not in pkmn_info['pokemon_list']:
    ...
    raid_number = pkmn_info['pokemon_list'].index(entered_raid) + 1
    raid_img_url = "http://floatzel.net/pokemon/black-white/sprites/images/{0}.png".format(str(raid_number))
    raid_embed = discord.Embed(title=_("Clembot! Click here for directions to the EX raid!"),url=raid_gmaps_link,description=_("Weaknesses: {weakness_list}").format(weakness_list=weakness_to_str(message.server, get_weaknesses(entered_raid))),colour=discord.Colour(0x2ecc71))
    raid_embed.set_thumbnail(url=raid_img_url)
    raidreport = await Meowth.send_message(message.channel, content = _("Clembot! {pokemon} EX raid reported by {member}! Details: {location_details}. Send proof of your invite to this EX raid to an admin and coordinate in {raid_channel}").format(pokemon=entered_raid.capitalize(), member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention),embed=raid_embed)
    await asyncio.sleep(1) #Wait for the channel to be created.

    raidmsg = _("""Clembot! {pokemon} EX raid reported by {member} in {citychannel}! Details: {location_details}. Coordinate here!
...
async def _raidegg(message):
    ...
    del args_split[0]
    if len(args_split) <= 1:
        await Meowth.send_message(message.channel, _("Clembot! Give more details when reporting! Usage: **!raidegg <level> <location>**"))
        return
    ...
    if args_split[0].isdigit():
        ...
        await Meowth.send_message(message.channel, _("Clembot! Give more details when reporting! Use at least: **!raidegg <level> <location>**. Type **!help** raidegg for more info."))
        return
    ...
    if egg_level > 5 or egg_level == 0:
        await Meowth.send_message(message.channel, _("Clembot! Raid egg levels are only from 1-5!"))
        return
    else:
        ...
        raid_embed = discord.Embed(title=_("Clembot! Click here for directions to the coming raid!"),url=raid_gmaps_link,description=_("Possible Bosses: {boss_list}").format(boss_list=boss_list),colour=discord.Colour(0x2ecc71))
        raid_embed.set_thumbnail(url=raid_img_url)
        raidreport = await Meowth.send_message(message.channel, content = _("Clembot! Level {level} raid egg reported by {member}! Details: {location_details}. Coordinate in {raid_channel}").format(level=egg_level, member=message.author.mention, location_details=raid_details, raid_channel=raid_channel.mention),embed=raid_embed)
        await asyncio.sleep(1) #Wait for the channel to be created.

        raidmsg = _("""Clembot! Level {level} raid egg reported by {member} in {citychannel}! Details: {location_details}. Coordinate here!
...
async def _eggassume(args, raid_channel):
    ...
    if config['allow_assume'][egglevel] == "False":
        await Meowth.send_message(raid_channel, _("Clembot! **!raid assume** is not allowed in this level egg."))
        return
    ...
    if entered_raid not in pkmn_info['pokemon_list']:
        ...
        if entered_raid not in pkmn_info['raid_list']:
            await Meowth.send_message(raid_channel, _("Clembot! The Pokemon {pokemon} does not appear in raids!").format(pokemon=entered_raid.capitalize()))
            return
        else:
            if get_number(entered_raid) not in raid_info['raid_eggs'][egglevel]['pokemon']:
                await Meowth.send_message(raid_channel, _("Clembot! The Pokemon {pokemon} does not hatch from level {level} raid eggs!").format(pokemon=entered_raid.capitalize(), level=egglevel))
                return
    ...
    if raidrole is None:
        ...
        await asyncio.sleep(0.5)
    await Meowth.send_message(raid_channel, _("Clembot! This egg will be assumed to be {pokemon} when it hatches!").format(pokemon=raidrole.mention))
    return
...
async def _eggtoraid(entered_raid, raid_channel):
    ...
    raid_number = pkmn_info['pokemon_list'].index(entered_raid) + 1
    raid_img_url = "http://floatzel.net/pokemon/black-white/sprites/images/{0}.png".format(str(raid_number))
    raid_embed = discord.Embed(title=_("Clembot! Click here for directions to the raid!"),url=raid_gmaps_link,description=_("Weaknesses: {weakness_list}").format(weakness_list=weakness_to_str(raid_channel.server, get_weaknesses(entered_raid))),colour=discord.Colour(0x2ecc71))
    raid_embed.set_thumbnail(url=raid_img_url)
    raidreportcontent = _("Clembot! The egg has hatched into a {pokemon} raid! Details: {location_details}. Coordinate in {raid_channel}").format(pokemon=entered_raid.capitalize(), location_details=egg_address, raid_channel=raid_channel.mention)
    await Meowth.edit_channel(raid_channel, name=raid_channel_name)
    raidmsg = _("""Clembot! The egg reported by {member} in {citychannel} hatched into a {pokemon} raid! Details: {location_details}. Coordinate here!
...
async def interested(ctx, *, count: str = None):
    ...
    if count:
        if count.isdigit():
            ...
            await Meowth.send_message(ctx.message.channel, _("Clembot! I can't understand how many are in your group. Just say **!interested** if you're by yourself, or **!interested 5** for example if there are 5 in your group."))
            return
    else:
...
async def coming(ctx, *, count: str = None):
    ...
    try:
        if server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['type'] == "egg":
            if server_dict[ctx.message.server]['raidchannel_dict'][ctx.message.channel]['pokemon'] == "":
                await Meowth.send_message(ctx.message.channel, _("Clembot! Please wait until the raid egg has hatched before announcing you're coming or present."))
                return
    except:
        pass
    ...
    if count:
        if count.isdigit():
            ...
            await Meowth.send_message(ctx.message.channel, _("Clembot! I can't understand how many are in your group. Just say **!coming** if you're by yourself, or **!coming 5** for example if there are 5 in your group."))
            return
    else:
        if ctx.message.author.mention in trainer_dict:
            count = trainer_dict[ctx.message.author.mention]['count']
...
async def here(ctx, *, count: str = None):
    ...
    if count:
        if count.isdigit():
            ...
            await Meowth.send_message(ctx.message.channel, _("Clembot! I can't understand how many are in your group. Just say **!here** if you're by yourself, or **!coming 5** for example if there are 5 in your group."))
            return
    else:
        if ctx.message.author.mention in trainer_dict:
            count = trainer_dict[ctx.message.author.mention]['count']
...
async def starting(ctx):
    ...
    starting_str = _("Clembot! The group that was waiting is starting the raid! Trainers {trainer_list}, please respond with {here_emoji} or **!here** if you are waiting for another group!").format(trainer_list=", ".join(ctx_startinglist), here_emoji=parse_emoji(ctx.message.server, config['here_id']))
    if len(ctx_startinglist) == 0:
        starting_str = _("Clembot! How can you start when there's no one waiting at this raid!?")
...
async def list(ctx):
    ...
    if ctx.invoked_subcommand is None:
        ...
        if checks.check_citychannel(ctx):
            ...
            listmsg += (_("Clembot! Current Raids for {0}:").format(cty.capitalize()))
            ...
            for r in server_dict[server]['raidchannel_dict']:
                ...
                if rc_d[r]['reportcity'] == cty and rc_d[r]['active'] and discord.utils.get(server.channels, id=r.id):
                    ...
                    activeraidnum += 1
            if activeraidnum == 0:
                await Meowth.send_message(channel, _("Clembot! No active raids! Report one with **!raid <name> <location>**."))
                return
            else:
...
@checks.activeraidchannel()
async def omw(ctx):
    await Meowth.send_message(ctx.message.channel, content = _("Clembot! Hey {member}, I don't know if you meant **!coming** to say that you are coming or **!list coming** to see the other trainers on their way").format(member=ctx.message.author.mention))
...
async def clearstatus(ctx):
    ...
    try:
        ...
        await Meowth.send_message(ctx.message.channel,"Clembot! Raid status lists have been cleared!")
    except KeyError:
        pass
...
async def duplicate(ctx):
        ...
        if author in t_dict:
            try:
                if t_dict[author]['dupereporter']:
                    dupeauthmsg = await Meowth.send_message(channel,_("Clembot! You've already made a duplicate report for this raid!"))
    ...
    if dupecount >= 3:
        rusure = await Meowth.send_message(channel,_("Clembot! Are you sure you wish to remove this raid?"))
...
async def location(ctx):
    ...
    if ctx.invoked_subcommand is None:
        ...
        newembed.set_thumbnail(url=oldembed['thumbnail']['url'])
        await Meowth.send_message(channel, content = _("Clembot! Here's the current location for the raid!\nDetails:{location}").format(location = location), embed = newembed)
...
async def new(ctx):
    ...
    if space1 == -1:
        await Meowth.send_message(message.channel, _("Clembot! We're missing the new location details! Usage: **!location new <new address>**"))
        return
    else:
...
async def _interest(ctx):
    ...
    listmsg = (_("Clembot! {trainer_count} interested{including_string}!").format(trainer_count=str(ctx_maybecount), including_string=maybe_exstr))
...
async def _otw(ctx):
    ...
    listmsg = (_("Clembot! {trainer_count} on the way{including_string}!").format(trainer_count=str(ctx_omwcount), including_string=otw_exstr))
    return listmsg
...
async def _waiting(ctx):
    ...
    listmsg = (_("Clembot! {trainer_count} waiting at the raid{including_string}!").format(trainer_count=str(ctx_waitingcount), including_string=waiting_exstr))
    return listmsg

@Meowth.command(pass_context=True, hidden=True)
@checks.activeraidchannel()
async def interest(ctx):
    await Meowth.send_message(ctx.message.channel, _("Clembot! We've moved this command to **!list interested**."))

@Meowth.command(pass_context=True, hidden=True)
@checks.activeraidchannel()
async def otw(ctx):
    await Meowth.send_message(ctx.message.channel, _("Clembot! We've moved this command to **!list coming**."))

@Meowth.command(pass_context=True, hidden=True)
@checks.activeraidchannel()
async def waiting(ctx):
    await Meowth.send_message(ctx.message.channel, _("Clembot! We've moved this command to **!list here**."))
...
async def invite(ctx):
    ...
    if ctx.message.attachments:
        ...
        wait_msg = await Meowth.send_message(ctx.message.channel,_("Clembot! I'll wait for you to send your pass!"))
        def check(msg):
        ...
        if invitemsg is not None:
            ...
            await Meowth.send_message(ctx.message.channel, "Clembot! You took too long to show me a screenshot of your invite! Retry when you're ready.")
            return
    ...
    if 'https://cdn.discordapp.com' in ctx.message.attachments[0]['url']:
        if 'png' in ctx.message.attachments[0]['url'].lower() or 'jpg' in ctx.message.attachments[0]['url'].lower():
            ...
            if 'EX Raid Battle' or "This is a reward" or "Please visit the Gym" in txt:
                ...
                if exraidcount > 0:
                    await Meowth.send_message(ctx.message.channel, "Clembot! {0}, it looks like you've got an EX Raid invitation! The following {1} EX Raids have been reported: \n {2} \n Reply with the number of the EX Raid you have been invited to. If none of them match your invite, type 'N' and report it with **!exraid**".format(ctx.message.author.mention, str(exraidcount), exraidlist))
                    reply = await Meowth.wait_for_message(author=ctx.message.author)
                    if reply.content.lower() == 'n':
                        await Meowth.send_message(ctx.message.channel, "Clembot! Be sure to report your EX Raid with **!exraid**!")
                    elif not reply.content.isdigit() or int(reply.content) > exraidcount:
                        await Meowth.send_message(ctx.message.channel, "Clembot! I couldn't tell which EX Raid you meant! Try the **!invite** command again, and make sure you respond with the number of the channel that matches!")
                        ...
                        await Meowth.send_message(ctx.message.channel, "Clembot! Alright {0}, you can now send messages in {1}! Make sure you let the trainers in there know if you can make it to the EX Raid!".format(ctx.message.author.mention, exraid_channel.mention))
                    else:
                        await Meowth.send_message(ctx.message.channel, "Clembot! I couldn't understand your reply! Try the **!invite** command again!")
                else:
                    await Meowth.send_message(ctx.message.channel, "Clembot! No EX Raids have been reported in this server! Use **!exraid** to report one!")
            else:
                await Meowth.send_message(ctx.message.channel, "Clembot! That doesn't look like an EX Raid invitation to me! If it is, please message an admin to get added to the EX Raid channel manually!")
        else:
            await Meowth.send_message(ctx.message.channel, "Clembot! Your attachment was not a supported image format!")
    else:
        await Meowth.send_message(ctx.message.channel, "Clembot! Please upload your screenshot directly to Discord!")
